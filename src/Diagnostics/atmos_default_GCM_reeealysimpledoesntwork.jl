...
#function squeeze(array,dim)
#    out = reshape(array, (size(arrray, 1), size(arrray, 2)) )
#    return out
#end
...
num_thermo(FT) = 2 #varsize(vars_thermo(FT))
#thermo_vars(array) = Vars{vars_thermo(eltype(array))}((squeeze(array[:,1,:],2), squeeze(array[:,2,:],2)))
thermo_vars(array) = Vars{vars_thermo(eltype(array))}(( array[:,1,:], array[:,2,:] ))

# Compute thermodynamic vars
function compute_thermo!(bl, localQ, localaux, thermoQ, nhorzelem, nvertelem, Nqk, Nq ,dg)
    ...
    th = thermo_vars(Array(thermoQ)) #
    #@info @sprintf("""air_temperature(ts) %s""", air_temperature(ts))
    #th.T = Array(air_temperature(ts))
    #th.θ_dry = Array(dry_pottemp(ts))
...
function compute_diagnosticsums_GCM!(
    ...
    @info @sprintf("""state_i.ρu[1] %s""", state_i.ρu[1] )
    ...
    ds.u .= all_state_data[:,:,:,2] ./ all_state_data[:,:,:,1]
    ds.v .= all_state_data[:,:,:,3] ./ all_state_data[:,:,:,1]
    ds.w .= all_state_data[:,:,:,4] ./ all_state_data[:,:,:,1]
    ...
    ds.T   .= all_thermo_data[:,:,:,1]
    ds.thd .= all_thermo_data[:,:,:,2]

    ds.vortrel .= all_dyn_data[:,:,:,3].*1000000.
...
function atmos_default_GCM_collect(dgngrp::DiagnosticsGroup, currtime)
    ...
    compute_thermo!(bl, localQ, localaux, thermoQ, nhorzelem, nvertelem, Nqk, Nq, dg)
    ...
    #dsumsi = [zeros(FT, num_diagnostic(FT)) for _ in 1:(Nqk * nvertelem)]

    #nlat = size(all_state_data,2)
    #nlon = size(all_state_data,3)
    #nlev = size(all_state_data,1)
    #@visitIQ nlat nlon nlev begin
    ...
    varvals = OrderedDict()
    #varnames = flattenednames(vars_diagnostic)
    @info @sprintf("""dsumsi[5,5,5,:] %s""", dsumsi[5,5,5,:])
    varnames = ["u","v","w","T","thd","vortrel"]
