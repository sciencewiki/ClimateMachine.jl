...
function compute_thermo!(bl, state, auxstate, thermoQ, dgngrp )
...
function atmos_default_GCM_collect(dgngrp::DiagnosticsGroup, currtime)
    ...
    thermoQ_pwise = [zeros(FT, num_thermo(FT)) for _ in 1:npoints, _ in 1:nrealelem]
    ...
    @visitQ nhorzelem nvertelem Nqk Nq begin
        ...
        compute_thermo!(bl, state, aux, ijk, e, thermoQ_pwise)
        thermoQ[ijk,: , e] = thermoQ_pwise
    end
