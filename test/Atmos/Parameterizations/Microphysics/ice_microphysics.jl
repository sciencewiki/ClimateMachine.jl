...
function kinematic_model_nodal_update_auxiliary_state!(
    ...
    aux.snow_w = terminal_velocity(param_set, snow_param_set, state.ρ,aux.q_sno)
    ...
    aux.src_snow_acnv = conv_q_ice_to_q_sno(
        param_set, ice_param_set, q, state.ρ, aux.T)
    ...
    aux.src_liq_snow_accr = accretion(
        param_set, liquid_param_set, snow_param_set, aux.q_liq, aux.q_sno,
        state.ρ)
    aux.src_ice_snow_accr = accretion(
        param_set, ice_param_set, snow_param_set, aux.q_ice, aux.q_sno,
        state.ρ)
    aux.src_ice_rain_accr = accretion(
        param_set, ice_param_set, rain_param_set, aux.q_ice, aux.q_rai,
        state.ρ)

    aux.src_rain_accr_sink = accretion_rain_sink(
        param_set, ice_param_set, rain_param_set, aux.q_ice, aux.q_rai, state.ρ)

    if aux.T < T_freeze
        aux.src_snow_rain_accr = accretion_snow_rain(
            param_set, snow_param_set, rain_param_set, aux.q_sno, aux.q_rai,
            state.ρ)
    else
        aux.src_snow_rain_accr = accretion_snow_rain(
            param_set, rain_param_set, snow_param_set, aux.q_rai, aux.q_sno,
            state.ρ)
    end
    ...
    aux.src_snow_subl = evaporation_sublimation(
        param_set, snow_param_set, q, aux.q_sno, state.ρ, aux.T)
...
function source!(
    ...
    # cloud ice + snow -> snow
    accr = ρ * accretion(
        param_set, ice_param_set, snow_param_set, q_ice, q_sno, state.ρ)
    source.ρq_ice -= accr
    source.ρq_tot -= accr
    source.ρq_sno += accr
    ...
    accr = ρ * accretion(
        param_set, liquid_param_set, snow_param_set, q_liq, q_sno, state.ρ)
    if T < _T_freeze
        source.ρq_liq -= accr
        source.ρq_tot -= accr
        source.ρq_sno += accr
    ...
    # cloud ice + rain -> snow
    accr = ρ * accretion(
        param_set, ice_param_set, rain_param_set, q_ice, q_rai, state.ρ)
    accr_rain_sink = ρ * accretion_rain_sink(
        param_set, ice_param_set, rain_param_set, q_ice, q_rai, state.ρ)
    source.ρq_ice -= accr
    source.ρq_tot -= accr
    ...
    if T < _T_freeze
        accr = ρ * accretion_snow_rain(
            param_set, snow_param_set, rain_param_set, q_sno, q_rai, state.ρ)
        source.ρq_sno += accr
        source.ρq_rai -= accr
    else
        accr = ρ * accretion_snow_rain(
            param_set, rain_param_set, snow_param_set, q_rai, q_sno, state.ρ)
        source.ρq_rai += accr
        source.ρq_sno -= accr
    end

    # rain -> vapour
    evap = ρ * evaporation_sublimation(
    ...
    subl = evaporation_sublimation(
        param_set, snow_param_set, q, q_sno, state.ρ, T)
    source.ρq_sno += subl
    source.ρq_tot -= subl
