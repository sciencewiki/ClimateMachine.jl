...
# 1. Know when to not use it
# 1. Contruct a column-wise linear solver with Conjugate Gradient
      ...
      for i4 in 1:tup[4]
        for i2 in 1:tup[2]
          for i1 in 1:tup[1]
            ...
            alias_y[i1, i2, :, i4, :, i6] .= reshape(tmp2, (tup[3], tup[5]))
          end
        end
      end
    end
...
# Now that we have this function, we can define a linear system that we will solve colmunwise
...
# 1. tup[2] is the number of Gauss–Lobatto points in the y-direction
# 1. tup[3] is the number of Gauss–Lobatto points in the z-direction
# 1. tup[4] is the number of states
# 1. tup[5] is the number of elements in the vertical direction
# 1. tup[6] is the number of elements in the other directions

# Now we define our linear operator as a random matrix.
Random.seed!(1235);
B = [randn(tup[3]*tup[5], tup[3]*tup[5]) for i1 in 1:tup[1], i2 in 1:tup[2], i4 in 1:tup[4], i6 in 1:tup[6] ];
columnwise_A = [B[i1,i2,i4,i6] * B[i1,i2,i4,i6]' + 10I for i1 in 1:tup[1], i2 in 1:tup[2], i4 in 1:tup[4], i6 in 1:tup[6] ];
columnwise_inv_A = [inv(columnwise_A[i1,i2,i4,i6]) for i1 in 1:tup[1], i2 in 1:tup[2], i4 in 1:tup[4], i6 in 1:tup[6] ];
columnwise_linear_operator! = closure_linear_operator!(columnwise_A, tup);
columnwise_inverse_linear_operator! = closure_linear_operator!(columnwise_inv_A, tup);
...
linearsolver = ConjugateGradient(x, max_iter= tup[3]*tup[5], dims = (3,5), reshape_tuple = tup);
...
# 1. Use a preconditioner if possible
# 1. Make sure that the linear system really is symmetric and positive-definite 
